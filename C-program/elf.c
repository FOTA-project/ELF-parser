...
/* MACROS */
#define   ELF_FILE          "bootloader-dummy-app.elf"
...
uint8_t Get_File_Size(uint32_t* ElfFileSize);
...
void main(void)
{
  ...
  if(IS_File_Exist() == OK)
  {
     ...
     if(IS_ElfFile(Elf_Header) == OK)
     {
        ...
        printf("Text_physical_address = 0x%X\n",Text_physical_address);
        printf("Data_physical_address = 0x%X\n",Data_physical_address);
        printf("Bss_physical_address = 0x%X\n\n",Bss_physical_address);
     }
     ...
     InfoFilePtr = fopen("INFO_FILE.txt" , "wb");
     fwrite(&InfoBuffer, 4, 5, InfoFilePtr);
...
uint8_t IS_ElfFile(Elf_Header_t *ElfFile)
{
	...
	if (!( (ElfFile->identity[0] == 0x7f) &&
          (ElfFile->identity[1] == 'E')  && 
          (ElfFile->identity[2] == 'L')  &&
          (ElfFile->identity[3] == 'F') ))
	{
      ElfFile_Status = NOT_OK;
...
uint8_t ElfFile_Load(uint8_t* ElfFileData ,	uint8_t ElfFileSize)
{
   ...
   {
      fread(ElfFileData,1, ElfFileSize, FilePtr);
      fclose(FilePtr);
   }
